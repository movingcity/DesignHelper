...
using System.Text;
using ResourcesClient.Controls.Support;
    ...
    public class QueryController
    {
        ...
        private void createFilterExpressionsAndFilteredCollection(out bool filterChanged, bool force)
        {
            ...
            if ((force && query.FilterString != String.Empty) || (query.FilterString != String.Empty && filterChanged))
            {
                ...
                // MODIFIED: VIKRAM: Need to observe the original source collection and not the CollectionView
                // else we have a recusion when applayFilter line (view.Filter = Pred...) raises the Reset event
                // and the filtering kicks off again recursively (via observable_CollectionChanged DOQuery line)
